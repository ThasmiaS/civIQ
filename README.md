...
* **Database:** Neon Serverless PostgreSQL with `pgvector` for instant semantic similarity mapping via 5 normalized SQLModel tables (Standard 384-dimensional vectors for reliable compatibility and robust precision).
* **Embeddings & LLM:** Llama-3.1-8B (Groq) for lightning-fast RAG inference on the backend, processing raw verbatim contexts generated via `fastembed` (BAAI/bge-small-en-v1.5) to ensure high semantic fidelity.
